...
#define LCD_XSIZE   IMAPFB_HRES
#define LCD_YSIZE   IMAPFB_HRES
...
extern struct Font * font_get(uint8_t w,uint8_t h);
...
void glcd_enable(uint8_t state)
{
	/* 打开背光 */
	/* 使能LCD本身 */
...
void glcd_clear(uint32_t color)
{
	...
	for (y = 0; y <= IMAPFB_VRES; y++)
	{
		for (x = 0; x <= IMAPFB_HRES; x++)
		{
			*p++ =color;  /* red */
		}
	}
