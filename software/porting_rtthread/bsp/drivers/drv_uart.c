...
static int imapx200_getc(struct rt_serial_device *serial)
{
    ...
    uart = (struct imap_uart *)serial->parent.user_data;
    while((uart->uart_device->LSR&(0x01<<0))==0);
	ch=uart->uart_device->DBUF&0xff;
...
void rt_hw_uart_init(void)
{
    ...
    uart->serial.int_rx = &uart->int_rx;
	    /* register UART3 device */
    rt_hw_serial_register(&uart->serial,
                          "uart3",
                          RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_INT_RX,
                          uart);
	rt_hw_interrupt_install(UART3_IRQ, rt_serial3_handler, RT_NULL, "UART3");
